...
class PresentationDirectTest < Minitest::Test
  def test_direct_load_presentation
    setup = presentation_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-presentation", setup[:live] ? "live" : "unit")
    if _should_skip
...
def presentation_direct_setup(mockres)
