...
require_relative 'AiPresentationGenerator_types'
...
class AiPresentationGeneratorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AiPresentationGeneratorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AiPresentationGeneratorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AiPresentationGeneratorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AiPresentationGeneratorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AiPresentationGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AiPresentationGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AiPresentationGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AiPresentationGeneratorError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AiPresentationGeneratorError => err
    ...
    if fetched.is_a?(Hash)
      status = AiPresentationGeneratorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AiPresentationGeneratorError.new(
        "graphql_error", "AiPresentationGeneratorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AiPresentationGeneratorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
