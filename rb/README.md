# AiPresentationGenerator Ruby SDK



The Ruby SDK for the AiPresentationGenerator API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Presentation` — with named operations (`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = AiPresentationGeneratorSDK.test(testopts, sdkopts)
```
...
| `Presentation` | `(data) -> PresentationEntity` | Create a Presentation entity instance. |
...
raise a `AiPresentationGeneratorError` (a `StandardError` subclass), so wrap
...
```
rb/
├── AiPresentationGenerator_sdk.rb       -- Main SDK module
...
The main module (`AiPresentationGenerator_sdk`) exports the SDK class
...
# presentation.data_get now returns the presentation data from the last load
