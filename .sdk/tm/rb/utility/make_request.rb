...
module AiPresentationGeneratorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AiPresentationGeneratorResponse.new({})
    ...
    if fetch_err
      ...
      response = AiPresentationGeneratorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AiPresentationGeneratorResponse.new(fetched)
    else
