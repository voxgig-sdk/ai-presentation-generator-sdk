...
class AiPresentationGeneratorMakeError
{
    public static function call(?AiPresentationGeneratorContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AiPresentationGeneratorContext([], null);
        }
        $op = $ctx->op ?? new AiPresentationGeneratorOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AiPresentationGeneratorResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AiPresentationGeneratorError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AiPresentationGeneratorError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof AiPresentationGeneratorError) {
