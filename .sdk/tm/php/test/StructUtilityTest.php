...
// AiPresentationGenerator SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AiPresentationGeneratorOmni::makeStructRunner(
    ...
    {
        return AiPresentationGeneratorOmni::objmodel(AiPresentationGeneratorOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return AiPresentationGeneratorOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return AiPresentationGeneratorOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AiPresentationGeneratorOmni::structfix($now, Struct::undef()))
                        !== json_encode(AiPresentationGeneratorOmni::structfix($val, Struct::undef()))) {
