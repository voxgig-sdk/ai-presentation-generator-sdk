...
//    only - a typed AiPresentationGeneratorContext there would make every ctx
...
final class AiPresentationGeneratorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AiPresentationGeneratorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new AiPresentationGeneratorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new AiPresentationGeneratorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new AiPresentationGeneratorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new AiPresentationGeneratorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?AiPresentationGeneratorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AiPresentationGeneratorError(is_string($code) ? $code : '', $msg);
    }
