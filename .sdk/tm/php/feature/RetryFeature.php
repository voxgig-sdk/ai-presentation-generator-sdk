...
// AiPresentationGenerator SDK retry feature
...
class AiPresentationGeneratorRetryFeature extends AiPresentationGeneratorBaseFeature
{
    ...
    public function init(AiPresentationGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AiPresentationGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AiPresentationGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AiPresentationGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
