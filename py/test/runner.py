# AiPresentationGenerator SDK test runner
...
class AiPresentationGeneratorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AiPresentationGeneratorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AiPresentationGeneratorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AiPresentationGeneratorTestRunner.getenv("AI_PRESENTATION_GENERATOR_TEST_LIVE")
        override = AiPresentationGeneratorTestRunner.getenv("AI_PRESENTATION_GENERATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AiPresentationGeneratorTestRunner.getenv(key)
        ...
        explain = AiPresentationGeneratorTestRunner.getenv("AI_PRESENTATION_GENERATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AiPresentationGeneratorTestRunner._test_control is not None:
            return AiPresentationGeneratorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AiPresentationGeneratorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AiPresentationGeneratorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AiPresentationGeneratorTestRunner.load_test_control()
...
def load_env_local():
    AiPresentationGeneratorTestRunner.load_env_local()


def env_override(m):
    return AiPresentationGeneratorTestRunner.env_override(m)


def entity_data(v):
    return AiPresentationGeneratorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AiPresentationGeneratorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AiPresentationGeneratorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AiPresentationGeneratorTestRunner.load_test_control()


def live_client_options():
    return AiPresentationGeneratorTestRunner.live_client_options()


def live_delay_ms():
    return AiPresentationGeneratorTestRunner.live_delay_ms()
