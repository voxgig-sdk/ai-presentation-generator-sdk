...
from aipresentationgenerator_sdk.aipresentationgenerator_types import (
    Presentation,
...
class PresentationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PresentationEntity(self._client, opts)
    ...
    def data_get(self) -> Presentation:
    ...
    def match_get(self) -> Presentation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Presentation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Presentation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: PresentationCreateData, ctrl=None) -> Presentation:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
