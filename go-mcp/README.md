...
[MCP](https://modelcontextprotocol.io) server exposing the AiPresentationGenerator SDK as
two agent tools — `ai-presentation-generator_list` and `ai-presentation-generator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ai-presentation-generator-mcp)
make build

# 2. Provide credentials via the environment
export AI_PRESENTATION_GENERATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ai-presentation-generator \
  -- /absolute/path/to/ai-presentation-generator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ai-presentation-generator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ai-presentation-generator_list: first page of records
{ "entity": "presentation" }
{ "entity": "presentation", "query": { } }

// ai-presentation-generator_load: one record by id
{ "entity": "presentation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ai-presentation-generator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ai-presentation-generator \
     -- "$PWD"/dist/*/ai-presentation-generator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ai-presentation-generator_list` and `ai-presentation-generator_load` tools now appear
   in new sessions. Ask the agent to *"list presentation using ai-presentation-generator"*
   and it calls `ai-presentation-generator_list` with `{"entity":"presentation"}`.
...
```sh
export AI_PRESENTATION_GENERATOR_APIKEY=sk_live_xxx            # API key
export AI_PRESENTATION_GENERATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ai-presentation-generator_list` tool
...
### Call the `ai-presentation-generator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ai-presentation-generator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ai-presentation-generator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AI_PRESENTATION_GENERATOR_APIKEY` | API key sent with every request. |
| `AI_PRESENTATION_GENERATOR_BASE` | Optional override of the API base URL. |
...
```sh
./ai-presentation-generator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ai-presentation-generator_load","arguments":{"entity":"presentation","query":{"id":1}}}}'
```
