# AiPresentationGenerator PHP SDK Reference

Complete API reference for the AiPresentationGenerator PHP SDK.
...
#### `AiPresentationGeneratorSDK::test($testopts = null, $sdkopts = null)`
...
```php
[$result, $err] = $client->Presentation()->create([
  "content" => /* `$STRING` */,
  "topic" => /* `$STRING` */,
]);
```
...
```php
[$result, $err] = $client->Presentation()->load(["id" => "presentation_id"]);
```
