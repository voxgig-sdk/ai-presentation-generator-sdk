...
// AiPresentationGenerator SDK feature test
//
...
class FtCtrl extends AiPresentationGeneratorControl
{
...
class FtClient
{
    ...
    private AiPresentationGeneratorUtility $utility;

    public function __construct(AiPresentationGeneratorUtility $utility, array $options)
    {
    ...
    public function get_utility(): AiPresentationGeneratorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AiPresentationGeneratorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AiPresentationGeneratorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AiPresentationGeneratorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AiPresentationGeneratorConfig::make_feature($name);
    ...
    private static function buildUrl(AiPresentationGeneratorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AiPresentationGeneratorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AiPresentationGeneratorResponse($fetched);
        }
    ...
    private function fail(AiPresentationGeneratorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AiPresentationGeneratorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
