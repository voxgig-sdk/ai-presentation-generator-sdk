...
local PresentationEntity = {}
PresentationEntity.__index = PresentationEntity


function PresentationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PresentationEntity)
  self._name = "presentation"
  self._client = client
...
function PresentationEntity:get_name()
  return self._name
end


function PresentationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PresentationEntity.new(self._client, opts)
end
...
function PresentationEntity:mark_deleted()
  self._deleted = true
end


function PresentationEntity:deleted()
  return true == self._deleted
end


function PresentationEntity:data_set(args)
  if args ~= nil then
...
function PresentationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PresentationEntity:match_set(args)
  if args ~= nil then
...
function PresentationEntity:match_get()
...
function PresentationEntity:stream(action, args, callopts)
...
---@param reqmatch PresentationLoadMatch
---@param ctrl? table
---@return Presentation
---@return string? err
function PresentationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata PresentationCreateData
---@param ctrl? table
---@return Presentation
---@return string? err
function PresentationEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function PresentationEntity:_run_op(ctx, post_done)
  local utility = self._utility
