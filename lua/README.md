# AiPresentationGenerator Lua SDK



The Lua SDK for the AiPresentationGenerator API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Presentation()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── ai-presentation-generator_sdk.lua    -- Main SDK module
...
The main module (`ai-presentation-generator_sdk`) exports the SDK constructor
