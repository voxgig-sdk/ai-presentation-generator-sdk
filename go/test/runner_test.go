...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("AI_PRESENTATION_GENERATOR_TEST_LIVE") == "TRUE" ||
		os.Getenv("AI_PRESENTATION_GENERATOR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("AI_PRESENTATION_GENERATOR_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.AiPresentationGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.AiPresentationGeneratorError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.AiPresentationGeneratorSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.AiPresentationGeneratorError{Msg: msg, Code: code}
}
