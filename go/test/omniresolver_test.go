...
import (
	...
	omni "github.com/voxgig-sdk/ai-presentation-generator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AiPresentationGeneratorError); is {
				out := map[string]any{
					"name":    "AiPresentationGeneratorError",
...
func omniCtx(arg any, client *sdk.AiPresentationGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AiPresentationGeneratorSDK); is {
			client = live
