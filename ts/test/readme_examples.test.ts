...
//     (\`new AiPresentationGeneratorSDK(...)\` / \`AiPresentationGeneratorSDK.test(...)\` become
//     \`AiPresentationGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AiPresentationGeneratorSDK'
...
const TEST_SEED = {"entity":{"presentation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AiPresentationGeneratorSDK(...)\` and any
// \`AiPresentationGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AiPresentationGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AiPresentationGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
