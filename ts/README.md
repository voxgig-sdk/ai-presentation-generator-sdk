...
`client.Presentation()` — each with a small set of operations (`load`, `create`)
...
| `tester(testopts?, sdkopts?)` | `AiPresentationGeneratorSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `AiPresentationGeneratorSDK.test(testopts?, sdkopts?)` | `AiPresentationGeneratorSDK` | Create a test-mode client. |
...
├── src/
│   ├── AiPresentationGeneratorSDK.ts        # Main SDK class
...
// presentation.match() returns { id: "example_id" }
```
