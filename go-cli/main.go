// Command AiPresentationGenerator-cli is a CLI + REPL for the AiPresentationGenerator SDK,
...
const prompt = "ai-presentation-generator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "presentation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AI_PRESENTATION_GENERATOR_APIKEY (api key), AI_PRESENTATION_GENERATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AI_PRESENTATION_GENERATOR_APIKEY carries the API key and
	// AI_PRESENTATION_GENERATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AI_PRESENTATION_GENERATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AI_PRESENTATION_GENERATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAiPresentationGeneratorSDK(opts)
