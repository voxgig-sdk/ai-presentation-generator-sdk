...
boru-driven command-line client **and** interactive REPL for the AiPresentationGenerator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ai-presentation-generator-cli)
make build

# 2. See usage (words, entities, env vars)
./ai-presentation-generator-cli --help

# 3. Provide credentials once, via the environment
export AI_PRESENTATION_GENERATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ai-presentation-generator-cli load 1 presentation            # {id:1} shorthand
./ai-presentation-generator-cli load '{id:1}' presentation       # explicit match map

# 5. Override the API base URL for a single call
AI_PRESENTATION_GENERATOR_BASE=https://api.example.com ./ai-presentation-generator-cli load 1 presentation

# 6. No arguments -> interactive REPL
./ai-presentation-generator-cli
ai-presentation-generator> load 1 presentation
ai-presentation-generator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ai-presentation-generator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ai-presentation-generator-cli load 1 presentation
   ```
...
```sh
./ai-presentation-generator-cli load 1 presentation          # scalar shorthand for {id:1}
./ai-presentation-generator-cli load '{id:1}' presentation     # explicit match map
```
...
./ai-presentation-generator-cli load 1 presentation
```
...
Run with no arguments to open a REPL (prompt `ai-presentation-generator>`). Each line is
evaluated as its own boru expression:

```text
$ ./ai-presentation-generator-cli
ai-presentation-generator> load 1 presentation
ai-presentation-generator> /help
ai-presentation-generator> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `presentation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ai-presentation-generator-cli`. |
...
(`./ai-presentation-generator-cli <expr>`) and interactively (the REPL), and expressions compose the
